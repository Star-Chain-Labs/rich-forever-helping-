...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center py-2 sm:px-[4rem] px-[1rem] shadow-md fixed z-30 w-full bg-white top-0 left-0">
      <div className="flex items-center gap-4 w-auto">
        <Link to={"/"}>
          <img
            src={MainContent.logo1}
            alt="logo"
            className="lg:w-[6rem] md:[5rem] w-14"
          />
        </Link>
      </div>
     ...
     <Link
        ...
        <button
          ...
          className="text-white text-xs md:text-sm flex items-center gap-2 bg-green-600  hover:bg-green-900 px-3 md:px-5 py-1.5 md:py-2 rounded-md shadow-md hover:shadow-lg transition"
        >
          {isLoggedIn ? (
            <>
              Dashboard <IoMdHome className="text-xl" />
            </>
          ) : (
            <>
